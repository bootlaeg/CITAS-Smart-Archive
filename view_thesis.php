...
// Check if user has access code for this thesis (unified on thesis_access table)
$has_access_code = false;
if (is_logged_in()) {
    $access_check = $conn->prepare("
        SELECT id FROM thesis_access WHERE user_id = ? AND thesis_id = ? AND status = 'approved'
    ");
    
    if ($access_check) {
        $uid = $_SESSION['user_id'];
        $access_check->bind_param("ii", $uid, $thesis_id);
...
<head>
    ...
    <link rel="stylesheet" href="css/view_thesis_redesign.css">
    <style>
        * {
            margin: 0;
            padding: 0;
...
<div class="container-wrapper">
    ...
    <aside class="thesis-sidebar">
        ...
        <!-- Thesis Info Section -->
        <div class="sidebar-section">
            <h3 class="sidebar-section-title"><i class="fas fa-user-circle"></i> Author Information</h3>
            <div class="sidebar-info-item">
                <span class="sidebar-info-label">Author</span>
                <span class="sidebar-info-value"><?php echo htmlspecialchars($thesis['author']); ?></span>
            </div>
            <div class="sidebar-info-item">
                <span class="sidebar-info-label">Course</span>
                <span class="sidebar-info-value"><?php echo htmlspecialchars($thesis['course']); ?></span>
            </div>
            <div class="sidebar-info-item">
                <span class="sidebar-info-label">Year</span>
                <span class="sidebar-info-value"><?php echo htmlspecialchars($thesis['year']); ?></span>
            </div>
        </div>

        <div class="sidebar-section">
            <h3 class="sidebar-section-title"><i class="fas fa-brain"></i> Classification</h3>
            <div class="sidebar-info-item">
                <span class="sidebar-info-label">Category</span>
                <span class="sidebar-info-value"><?php echo !empty($thesis['subject_category']) ? htmlspecialchars($thesis['subject_category']) : 'N/A'; ?></span>
            </div>
            <div class="sidebar-info-item">
                <span class="sidebar-info-label">Keywords</span>
        ...
        <div class="sidebar-section">
            <h3 class="sidebar-section-title"><i class="fas fa-key"></i> Research Method</h3>
            <div class="sidebar-info-item">
                <span class="sidebar-info-label">Research Type</span>
                <span class="sidebar-info-value"><?php echo !empty($thesis['research_method']) ? htmlspecialchars($thesis['research_method']) : 'Not specified'; ?></span>
            </div>
        </div>

        <div class="sidebar-section">
            <h3 class="sidebar-section-title"><i class="fas fa-book"></i> Publication Details</h3>
            <div class="sidebar-info-item">
                <span class="sidebar-info-label">Views</span>
                <span class="sidebar-info-value"><?php echo $thesis['views'] + 1; ?></span>
            </div>
            <div class="sidebar-info-item">
                <span class="sidebar-info-label">Status</span>
                <span class="sidebar-info-value"><?php echo htmlspecialchars(ucfirst($thesis['status'])); ?></span>
            </div>
        </div>
    </aside>
...
<?php if (!(is_admin() || (isset($_SESSION['user_role']) && $_SESSION['user_role'] === 'instructor') || $has_access_code)): ?>
        <div class="access-required-container" style="text-align: center; padding: 4rem 2rem; background: #fff8f5; border: 1px solid #ffcdb2; border-radius: 12px; margin: 2rem 0;">
            <div style="width: 64px; height: 64px; background: rgba(255, 107, 53, 0.1); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 1.5rem auto;">
                <i class="fas fa-lock" style="color: #ff6b35; font-size: 1.8rem;"></i>
            </div>
            <h3 style="color: #d4400a; margin-bottom: 0.5rem; font-size: 1.4rem;">Thesis Access Required</h3>
            <p style="color: #666; max-width: 500px; margin: 0 auto 1.5rem auto; line-height: 1.5;">To view the IMRaD formatted journal version and use the AI chatbot, you need to request access from the administrator.</p>
            <button id="imradRequestAccessBtn" class="btn-request-access" onclick="requestThesisAccess(this)" style="background: linear-gradient(135deg, #ff6b35, #e85d2c); color: #fff; border: none; padding: 0.8rem 2rem; border-radius: 8px; font-size: 1rem; font-weight: 600; cursor: pointer; transition: all 0.2s ease; box-shadow: 0 4px 12px rgba(255, 107, 53, 0.3);">
                <i class="fas fa-key" style="margin-right: 0.5rem;"></i>Request Access
            </button>
        </div>
<?php else: ?>
                <?php if (!empty($thesis['is_journal_converted']) && !empty($thesis['journal_file_path'])): ?>
                    <div class="document-section" style="margin-top: 0;">
                ...
                <?php else: ?>
                    <div class="coming-soon" style="min-height: 300px;">
                        ...
                        <h3 class="coming-soon-title">No Journal Version Available</h3>
                        <p class="coming-soon-description">
                            This thesis has not been converted to journal format yet. The journal converted version will appear here once the IMRaD conversion is completed.
                        </p>
                    </div>
                <?php endif; ?>
<?php endif; ?>
...
// Request thesis access (unified — grants document viewer + chatbot)
// Used by both the chatbot overlay and the IMRaD tab button
function requestThesisAccess(btnElement) {
    const originalText = btnElement.innerHTML;
    btnElement.disabled = true;
    btnElement.innerHTML = '<i class="fas fa-spinner fa-spin me-2"></i>Sending request...';
    ...
    .then(data => {
        if (data.success) {
            btnElement.innerHTML = '<i class="fas fa-check me-2"></i>Request sent!';
